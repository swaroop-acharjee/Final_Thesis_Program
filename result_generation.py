...
for file_name in file_lst:
    ...
    for P, Q, R in hyperparameters:
            ...
            for j in range(Z):
                for k in range(Y):
                    for l in range(X):
                        ...
                        for r in range(N):
                            su += (
                                pow(global_u[r][j][k][l], P)
                                * pow(local_u[r][j][k][l], Q)
                                * pow(type_2_u_nrm[r][j][k][l], R)
                            )

                        final_u[i][j][k][l] = (
                            pow(global_u[i][j][k][l], P)
                            * pow(local_u[i][j][k][l], Q)
                            * pow(type_2_u_nrm[i][j][k][l], R)
                        ) / su
	  ...
	  """
        ...
        avg_sa = sum_sa / 4
        error_percentage = (
            mis_classified / (correct_classified + mis_classified)
        ) * 100
